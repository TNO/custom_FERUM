function [b,fval,exitflag,output] = my_fzero_gprimefun_vectorized(x,tolx,Uprime1,R1,lsf,probdata,analysisopt,gfundata,femodel,randomfield)
...
if (size(x,1) == 2)
    ...
    fa = gprimefun(a,Uprime1,R1,lsf,probdata,analysisopt,gfundata,femodel,randomfield);
    fb = gprimefun(b,Uprime1,R1,lsf,probdata,analysisopt,gfundata,femodel,randomfield);
    fcount = fcount + 2*nx;
...
elseif (size(x,1) == 1)
    ...
    fx = gprimefun(x,Uprime1,R1,lsf,probdata,analysisopt,gfundata,femodel,randomfield);
    fcount = fcount + 1*nx;
    ...
    while any ( (fa > 0) == (fb > 0) )
        ...
        a = x - dx;  fa = gprimefun(a,Uprime1,R1,lsf,probdata,analysisopt,gfundata,femodel,randomfield);
        fcount = fcount + 1*nx;
        ...
        b = x + dx;  fb = gprimefun(b,Uprime1,R1,lsf,probdata,analysisopt,gfundata,femodel,randomfield);
        fcount = fcount + 1*nx;        
...
output.funcCount = fcount;
output.algorithm = 'bisection, interpolation';
fval = gprimefun(b,Uprime1,R1,lsf,probdata,analysisopt,gfundata,femodel,randomfield);
fcount = fcount + 1*nx;
