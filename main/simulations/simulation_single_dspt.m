function [ simulationresults, probdata ] = simulation_single_dspt(lsf,probdata,analysisopt,gfundata,femodel,randomfield)
...
while k < num_sim
   ...
   if cov_of_q_bar(k) <= target_cov, break, end
...
if sum_q > 0
   ...
   if echo_flag
      ...
      %figure
      npts = 200;
      if k/block_size > npts
         %plot( [1 round((1:npts)/npts*k/block_size)*block_size], cov_of_q_bar( [1 round((1:npts)/npts*k/block_size)*block_size] ), 'b.' );
         if cov_of_q_bar(round(1/npts*k/block_size)*block_size) > 0
            %axis([0 k 0 cov_of_q_bar(round(1/npts*k/block_size)*block_size)]);
         end
      else
         %plot( [1 block_size:block_size:k], cov_of_q_bar([1 block_size:block_size:k]), 'b.' );
         if cov_of_q_bar(block_size) > 0
            %axis([0 k 0 cov_of_q_bar(block_size)]);
         end
      end
      ...
      %npts = 200;
      if k/block_size > npts
         %plot( [1 round((1:npts)/npts*k/block_size)*block_size], q_bar( [1 round((1:npts)/npts*k/block_size)*block_size] ), 'b.' );
         %hold on
      else
         %plot( [1 block_size:block_size:k], q_bar([1 block_size:block_size:k]), 'b.' );
         hold on
      end
      %title('Probability of failure (p_f)')
      %xlabel('Number of simulations');
