...
% Reference value for pf: pf = 1.73e-003
...
probdata.marg = [ 2*ones(n,1) ones(n,1) sig*ones(n,1) ones(n,1) nan*ones(n,4) zeros(n,1) ];

% Correlation matrix
probdata.correlation = eye(n);
...
analysisopt.block_size           = 25000;    % Number of g-calls to be sent simultaneously
...
gfundata(1).expression = [ '-(x' sprintf('%d+x', 1:(n-1)) sprintf('%d-%.10e)',n,n+a*sig*n^0.5) ];
