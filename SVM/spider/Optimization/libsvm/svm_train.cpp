...
//Could not find a match for 'Kernel::Kernel(const int,svm_node * * const,const svm_parameter)' in function SVR_Q::SVR_Q(const svm_problem &,const svm_parameter &)
...
svm_model *svm_train(const svm_problem *prob, svm_parameter *param)
{
  svm_model *model;
  ...
  if(param->svm_type == ONE_CLASS ||
  ...
  else
    {
      ...
      int max_nr_class = 16;
      int nr_class = 0;
      int *label = Malloc(int,16);
      int *count = Malloc(int,16);
	...
	{	
	  ...
	  for(j=0;j<nr_class;j++)
	    if(param->weight_label[i] == label[j])
	      break;
	  if(j == nr_class)
	    cerr << "warning: class label %d specified in weight is not found" << param->weight_label[i] << "\n";
	  else
...
{
  // Declarations
  svm_parameter param; //store the params (C, kernel, etc...)		
  svm_problem prob;    //store the problem (data)
  svm_model *model;    //store the model (Xsv, b,...)
  svm_node *x_space;   //store just one vector
  int nr_fold;         // ??
  ...
  // The function must have 16 arguments corresponding to all the options
  if (nrhs != 16) {
    mexErrMsgTxt("1-Usage: [alpha,b,Xsv,epsilon,C] "
		 "=train(X,Y,svm_type,kernel_type,degree,gamma,coef0,nu,cache_size,...\n"
		 "C,eps,epsilon,shrinking,nr_weight,weight_label,weight); ");
    return;
  }
    	
	
  // The output must have at least 3 terms
  if (nlhs>=3)
    {
      m = mxGetM(prhs[0]);// number of lines
      d = mxGetN(prhs[0]);// number of columns    
      
      plhs[2]=mxCreateDoubleMatrix(m,d,mxREAL); // for Xsv
      xsv = mxGetPr(plhs[2]);// get the double pointer from the mxarray         
    }
  
  
  plhs[0] = mxCreateDoubleMatrix(m, 1, mxREAL); // for alpha	    
  alpha = mxGetPr(plhs[0]);// same as above    
  ...
  // for the dataset
  prob.l = m; // l is also the number of lines
  elements = m*(d+1);// total number of elements
  Xtmp = mxGetPr(prhs[0]);// the input matrix
  prob.y = mxGetPr(prhs[1]);// targets      
  prob.x = Malloc(svm_node *,m); // inputs
  x_space = Malloc(svm_node,elements);// to build inputs
  
  max_index = d;
  j=0;
  // build prob.x (put -1 at the end of each line)
  for(i=0;i<m;i++)
    {
      prob.x[i] = &x_space[i*(d+1)];
      for(j=0;j<=d;j++)
	{	  
	  x_space[i*(d+1)+j].index=j+1;
	  x_space[i*(d+1)+j].value=Xtmp[j*d+i];
	}	
      x_space[i*(d+1)+d].index= -1;
    }
  ...
  // type of svm
  param.svm_type =(int)*mxGetPr(prhs[2]);
  // type of kernel
  param.kernel_type = (int)*mxGetPr(prhs[3]);	
  // degree of poly
  param.degree = (int)*mxGetPr(prhs[4]);
  // scaling coeff
  param.gamma = *mxGetPr(prhs[5]);
  // bias coeff
  param.coef0 = *mxGetPr(prhs[6]);
  // nu
  param.nu = *mxGetPr(prhs[7]);
  // cache_size
  param.cache_size = *mxGetPr(prhs[8]);;
  // C
  param.C = *mxGetPr(prhs[9]);;
  // precision
  param.eps = *mxGetPr(prhs[10]);;
  // epsilon
  param.p = *mxGetPr(prhs[11]);;
  // shrinking
  param.shrinking = (int)*mxGetPr(prhs[12]);;
  // nr_weight
  param.nr_weight = (int)*mxGetPr(prhs[13]);;
  // not used?
  ...
  tmp1=(double *)mxGetPr(plhs[1]);
  tmp2=(double *)mxGetPr(plhs[3]);
  tmp3=(double *)mxGetPr(plhs[4]);
  tmp1[0] = model->rho[0];
  tmp2[0] = param.p;
  tmp3[0] = param.C;
}
