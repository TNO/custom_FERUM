...
analysisopt.sampling             = 1;        % sampling = 1: Sobol' indices are assessed from Crude Monte Carlo (CMC) simulations
                                             %            2: Sobol' indices are assessed from Quasi Monte Carlo (QMC) simulations 
analysisopt.Sobolsetopt          = 0;        % Sobolsetopt = 0: Sobol' sequences from Matlab Statistic toolbox
                                             %               1: Sobol' sequences from Sobol' ( Broda - http://www.broda.co.uk/ )
analysisopt.SVR                  = 'yes';    % SVR = 'yes': a SVR is built as a surrogate to the physical model. Sobol' indices are assessed on the SVR surrogate
                                             %       'no':  no SVR surrogate is built. Sobol' indices are assessed on the physical model, by means of gfundata data structure
...
analysisopt.total_indices        = 1;        % total_indices = 1: assessment of total indices
                                             %                 0: no assessment
analysisopt.all_indices          = 1;        % all_indices = 1: assessment of first order, second order,..., all order indices (analysisopt.first_indices must be set to 1)
                                             %               0: no assessment
analysisopt.NbCal                = 30;       % Number of replications of Sobols'indices assessments (based on the same SVR surrogate, if analysisopt.SVR = 'yes') 
                                             %    NbCal = 1: one single assessment
                                             %    NbCal > 1: assessment of mean and variance for each Sobols'indice

%  Global Sensitivity Analysis (GSA)- SVR options
analysisopt.SVRbasis             = 'CVT_unif'; % SVRbasis = 'Sobol_norm': Points are deterministically generated in the standard space (normal distribution)
                                               %                          (use of Sobol' sequences in [0 1] hypercube)
                                               %            'CVT_unif'  : Points are deterministically generated in an hypersphere with a specific radius
...
% However, the user may have some knowledge of the problem in order to define a grid search for the hyperparameters (C, epsilon, sigma)
analysisopt.n_reg_radius         = 200000;   % Number of samples for assessing the radius of the learning hypersphere, when analysisopt.SVRbasis = 'CVT_unif'
nrv = size(probdata.marg,1);
analysisopt.buf_size = round(5e6/nrv);       % RAM-size dependent parameters for eval_svm.m function
analysisopt.svm_buf_size = 3000^2;           % RAM-size dependent parameters for eval_svm.m function
...
gfundata(1).expression = ' [ -0.5 * ( x1.^2 + x2.^2 + x3.^2 - 2*x1.*x2 - 2*x2.*x3 - 2*x3.*x1 ) - (x1+x2+x3)/3^0.5 + 3 ; -0.5 * ( x1.^2 + x2.^2 + x3.^2 ) - (x1+x2+x3)/3^0.5 + 3 ; -0.5 * ( x1.^2 + x2.^2 + x3.^2 - 2*x1.*x2 - 2*x2.*x3 - 2*x3.*x1 ) - (x1+x2+x3)/3^0.5 + 3 ] ';
% gfundata(1).expression = '-0.5 * ( x1.^2 + x2.^2 + x3.^2 - 2*x1.*x2 - 2*x2.*x3 - 2*x3.*x1 ) - (x1+x2+x3)/3^0.5 + 3';
