function [ formresults, probdata ] = form_v2(lsf,probdata,analysisopt,gfundata,femodel,randomfield)
...
%     keyboard
   if echo_flag
      disp('..................................')
      disp('Now carrying out iteration number:'),disp(ii)
   end
   ...
   if ~isempty(id)
       ...
       if strcmp(last_warn, 'MATLAB:illConditionedMatrix')
           conv_flag = 1;
   ...
   disp('Calculation of the gradient...')
...
%    [ G, grad_g ] = gfun(lsf,x,grad_flag,probdata,analysisopt,gfundata,femodel,randomfield);
   [ G, grad_g ] = gfun_v2(lsf,x,grad_flag,probdata,analysisopt,gfundata,femodel,randomfield);
   disp('Calculation of the gradient has completed.')
   
   grad_G = (grad_g' * J_x_u)';
   Recorded_G_function_values(ii) = G;
...
%    grad_g
   ...
   % Check convergence
   if ( (abs(G/Go)<e1) & (norm(u-alpha'*u*alpha)<e2) ) | ii==i_max
      conv_flag = 1;
   end
   Recorded_G_criterion(ii) = abs(G/Go);
   Recorded_u_criterion(ii) = norm(u-alpha'*u*alpha);
   if Recorded_u_flag
   ...
   if conv_flag == 0;
      ...
      % Determine step size
      if step_code == 0
         disp('Calculation of the next step size...')
         step = step_size_multiproc_v2(lsf,G,grad_G,u,d,probdata,analysisopt,gfundata,femodel,randomfield);
         disp('Calculation of the next step size has completed.')
      else
         step = step_code;
      end
      Recorded_step_size_values(ii) = step;
...
%       end
      ...
      Recorded_beta(ii) = alpha' * u;
      
      % Display diagnostics
      disp('---------------------------------------------')
      disp('Recorded_G_criterion:')
      disp(Recorded_G_criterion)
      disp('Recorded_u_criterion:')
      disp(Recorded_u_criterion)
      disp('Recorded_beta:')
      disp(Recorded_beta)
      disp(['FORM iteration #',num2str(ii-1),' has completed.'])
      disp('=============================================')
   end
...
%  Post-processing 
formresults.iter   = ii;                                                 % Number_of_iterations
beta               = alpha' * u;
...
formresults.e2     = Recorded_u_criterion;                              % Recorded values og u-criterion
if Recorded_u_flag
